...
import {
  ...
  AlertTriangle, Loader2, Download, Eye, X, EyeOff, ChevronDown,
} from "lucide-react";
...
function isTimberWood(item) {
  ...
  return /timber|teak|sagwan|hardwood/i.test(text) && !isWoodenCladding(item) && !isWoodenBaluster(item);
}
function isWoodenCladding(item) {
  const text = `${item?.materialType || ""} ${item?.product || ""} ${item?.category || ""}`.toLowerCase();
  return /cladding/i.test(text);
...
function parseSolidSurfaceDimensions(name) {
  ...
  if (isNaN(w) || isNaN(h) || w === 0 || h === 0) return null;
  let wFeet, hFeet;
  if (w > 50 || h > 50) { wFeet = w / 12; hFeet = h / 12; }
...
function calculateByUnit(item) {
  ...
  const getCustomLength = () => {
    const lft = parseFloat(item.customLength || 0);
    const lin = parseFloat(item.customLengthIn || 0);
    return lft + (lin / 12);
  };
  const getCustomWidth = () => {
    const wft = parseFloat(item.customWidth || 0);
    const win = parseFloat(item.customWidthIn || 0);
    return wft + (win / 12);
  };

  if (isSolidSurface(item)) {
    const customL = getCustomLength();
    const customW = getCustomWidth();
    let areaPerPc = item.areaPerPiece || 0;
    if (customL > 0 && customW > 0) areaPerPc = customL * customW;
    const totalSqft = areaPerPc * qty;
      ...
      {isOpen && !disabled && (
        <div className="ss-dropdown" style={{ background: T?.cardBg, borderColor: T?.borderSoft }}>
          {filtered.length > 0 && filtered.map((o, idx) => (
            <div key={`ss-opt-${getVal(o)}-${idx}`} className={`ss-option ${getVal(o) === value ? "selected" : ""}`}
              onClick={() => commit(getVal(o), getDisp(o))} style={{ color: T?.textDark }}>
              {getDisp(o)}
            </div>
          ))}
          {allowCustom && inputValue.trim() && !exactMatch && (
...
function getChallanPrintHTML(order, challan, hidePrice = false, chargesList = [], gstRate = 0, gstAmount = 0) {
  ...
  const itemRowsArr = regularItems.map((it) => {
    ...
    if (it.size) {
      ...
      const productLower = (it.product || '').replace(/\s/g, '').toLowerCase();
      const sizeVariations = [
        sizeWithoutSpaces, sizeWithoutSpaces.replace(/×/g, 'x'), sizeWithoutSpaces.replace(/x/g, '×'),
        sizeWithoutSpaces.replace(/"/g, ''), sizeWithoutSpaces.replace(/×/g, 'x').replace(/"/g, ''),
        sizeWithoutSpaces.replace(/x/g, '×').replace(/"/g, ''),
      ];
      const sizeAlreadyInName = sizeVariations.some(v => v && productLower.includes(v));
  ...
  const pages = [];
  if (itemRowsArr.length === 0) pages.push([]);
  else {
    for (let i = 0; i < itemRowsArr.length; i += ITEMS_PER_PAGE) pages.push(itemRowsArr.slice(i, i + ITEMS_PER_PAGE));
  }
  let chargesHtml = "";
  if (chargesList.length > 0 && !hidePrice) {
    chargesHtml = `<div class="ktp-charge-section"><div class="ktp-charge-header">ADDITIONAL CHARGES</div>${chargesList.map((ch) => `<div class="ktp-charge-row"><span class="ktp-charge-name">${ch.name}</span><span class="ktp-charge-amt">₹${ch.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>`).join("")}</div>`;
  ...
  const pagesHTML = pages.map((pageRows, pageIdx) => {
    ...
    let spacerCells = "";
    for (let j = 0; j < colCount; j++) spacerCells += `<td>&nbsp;</td>`;
    emptyRows += `<tr class="spacer-row">${spacerCells}</tr>`;
    return `<div class="page-wrapper"><div class="page-content"><div class="ktp-header"><div class="ktp-logo-circle"><img src="/logo.jpeg" alt="KTP" /></div><div class="ktp-header-center"><div class="ktp-brand-name">Krishna</div><div class="ktp-brand-sub">Timber &amp; Plywoods</div><div class="ktp-brand-addr">${SHOP_INFO.address} &nbsp;|&nbsp; Ph.: ${SHOP_INFO.phone}, ${SHOP_INFO.phone2}</div></div><div class="ktp-header-right-space"></div></div><div class="ktp-meta"><div class="ktp-meta-left"><div class="ktp-since">Chhabra's Since 1979</div><div class="ktp-gstin">GSTIN : ${SHOP_INFO.gstin}</div><div class="ktp-cust-gst">Cust. GST No.: <span class="ktp-cust-gst-value">${order.gstCustomerName || ""}</span></div></div><div class="ktp-dc-box"><div class="ktp-dc-title">DELIVERY CHALLAN</div><div class="ktp-dc-details">No.: <strong>${challan.challanNo}</strong> &nbsp;&nbsp;&nbsp; Date: <strong>${new Date(challan.challanDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</strong></div></div></div><div class="ktp-info">${consigneeInfoHTML}</div><div class="ktp-table-wrap"><table class="items"><thead><tr><th style="width:35px">S.No.</th><th class="tl">Description of Goods</th>${!hidePrice ? `<th style="width:95px">Quantity</th><th style="width:80px">Rate</th><th style="width:95px">Amount</th>` : `<th style="width:100px">Quantity</th>`}</tr></thead><tbody>${pageRows.join("")}${emptyRows}</tbody></table></div><div class="ktp-footer"><div class="ktp-footer-left"><div class="ktp-terms-section"><div class="ktp-footer-cert">Certified that the particulars given above are true and correct.</div><ul class="ktp-terms-list"><li>Goods once sold will not be taken back or exchanged.</li><li>All disputes are subject to Bhopal jurisdiction only.</li><li>Interest @2% per month will be charged on overdue payments.</li></ul></div><div class="ktp-sig-area"><div class="ktp-sig-box"><div class="ktp-sig-line"></div><div class="ktp-sig-label">Customer Signature</div></div><div class="ktp-sig-box"><div class="ktp-footer-for-inline">For : Krishna Timber &amp; Plywoods</div><div class="ktp-sig-line"></div><div class="ktp-sig-label">Authorised Signatory</div></div></div></div><div class="ktp-footer-right">${footerRight}</div></div></div></div>`;
  }).join("");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Challan ${challan.challanNo}</title><style>${PRINT_CSS}</style></head><body><div class="action-bar"><button class="action-btn btn-print" onclick="window.print()">🖨️ Print Challan</button><button class="action-btn btn-save" onclick="savePDF()">💾 Save as PDF</button><button class="action-btn btn-close" onclick="window.close()">✕ Close</button></div>${pagesHTML}<script>function savePDF(){var ab=document.querySelector('.action-bar');if(ab)ab.style.display='none';window.print();setTimeout(function(){if(ab)ab.style.display='flex';},1200);}</script></body></html>`;
}
...
export default function ChallanOnlyBilling() {
  ...
  function createEmptyItem(ov = {}) {
    return {
      ...
      areaPerPiece: null,
      customLength: "", customLengthIn: "", customWidth: "", customWidthIn: "",
      dfSectionWidth: "", dfSectionThickness: "", dfHeightFt: "", dfHeightIn: "",
      dfWidthFt: "", dfWidthIn: "", dfVerticalQty: '', dfHorizontalQty: '',
      ...ov,
    };
  }
  function createEmptyGroup() {
  ...
  const genChallanNo = () => {
    const y = new Date().getFullYear();
    const prefix = `CHL-${y}-`;
    const max = challans.filter((c) => c.challanNo?.startsWith(prefix))
      .reduce((m, c) => Math.max(m, parseInt(c.challanNo.replace(prefix, "")) || 0), 0);
    return `${prefix}${String(max + 1).padStart(4, "0")}`;
  };
    ...
    setItemGroups((prev) => prev.map((g) => {
      ...
      return {
        ...
        items: g.items.map((item) => {
          ...
          if (field === "skuCode") {
            ...
            if (found) {
              ...
              if (isSolidSurface(u)) {
                ...
                const dims = parseSolidSurfaceDimensions(found.materialName);
                if (dims) {
                  u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness;
                  u.size = `${dims.widthOrig}"×${dims.heightOrig}"`;
                  u.customLength = dims.widthFeet; u.customLengthIn = "";
                  u.customWidth = dims.heightFeet; u.customWidthIn = "";
                }
              } else if (isCeilingPlank(u)) {
                u.unit = "SQFT";
                const dims = parseSheetDimensions(found.materialName);
                ...
                const dims = parseBalusterDimensions(found.materialName);
                if (dims) {
                  u.width = dims.width; u.thickness = dims.thickness;
                  u.lengthFeet = dims.lengthFeet; u.lengthInches = "";
                  u.size = `${dims.width}"×${dims.thickness}" - ${dims.lengthFeet} ft`;
                }
              } else if (isWoodenCladding(u)) {
                u.unit = "Per Piece";
                ...
                const dims = parseDoorFrameDimensions(found.materialName);
                if (dims) {
                  u.dfSectionWidth = dims.sectionWidth; u.dfSectionThickness = dims.sectionThickness;
                  u.dfHeightFt = dims.heightFt; u.dfHeightIn = dims.heightIn;
                  u.dfWidthFt = dims.widthFt; u.dfWidthIn = dims.widthIn;
                }
              } else if (isSheet) {
                u.unit = "SQFT";
                const dims = parseSheetDimensions(found.materialName);
          ...
          if (["customLength", "customLengthIn", "customWidth", "customWidthIn"].includes(field)) {
            u[field] = val;
            const lft = parseFloat(u.customLength || 0);
            const lin = parseFloat(u.customLengthIn || 0);
            const wft = parseFloat(u.customWidth || 0);
            const win = parseFloat(u.customWidthIn || 0);
            const totalL = lft + (lin / 12);
            const totalW = wft + (win / 12);
            if (totalL > 0 && totalW > 0) {
              const lDisplay = lin > 0 ? `${lft}'${lin}"` : `${lft}'`;
              const wDisplay = win > 0 ? `${wft}'${win}"` : `${wft}'`;
              u.size = `${lDisplay}×${wDisplay}`;
              u.areaPerPiece = totalL * totalW;
            }
          }
    ...
    try {
      ...
      const validItems = getAllItems()
        ...
        .map((it) => ({
          ...
          lengthDisplay: it.isWood ? `${it.lengthFeet || 0}'-${it.lengthInches || 0}"` : "",
          specification: it.specification || "",
          isSheet: it.isSheet, areaPerPiece: it.areaPerPiece,
          pieces: parseFloat(it.quantity || 0),
          sentQty: parseFloat(it.quantity || 0),
          orderedQty: it.calculatedQty,
        }));
      const validCharges = charges.filter((c) => c.chargeName && c.amount > 0).map((ch) => ({
      ...
      if (challanForm.gstRate > 0 && gstAmount > 0) {
        validCharges.push({
          name: `GST (${challanForm.gstRate}%)`, type: "gst", unit: "Percentage",
          quantity: challanForm.gstRate, rate: challanForm.gstRate, amount: gstAmount,
        });
      }
      const payload = {
        challan: {
  ...
  const handleSaveToSheet = async (item, group) => {
    if (!item.product || item.skuCode) { setError("Yeh product already saved hai ya empty hai"); return; }
    if (!confirm(`"${item.product}" ko Dropdown sheet mein save karna hai?`)) return;
    ...
    try {
      ...
      if (res.success) {
        ...
        if (pR.success) setProducts(pR.data);
        const newProduct = res.data?.[0];
        if (newProduct?.skuCode) updateGroupItem(group.groupId, item.uid, "skuCode", newProduct.skuCode);
        alert(`✅ Product saved! SKU: ${newProduct?.skuCode || "Generated"}`);
    ...
    if (savedItems.length > 0) {
      const groupItems = savedItems.map(it => {
        ...
        return createEmptyItem({
          ...
          quantity: it.sentQty || it.quantity || "",
          rate: it.rate || "", amount: parseFloat(it.amount) || 0,
          calculatedQty: parseFloat(it.calculatedQty) || 0,
  ...
  const getCategoriesFor = (mt) => [...new Set(products.filter((p) => !mt || p.materialType === mt).map((p) => p.category).filter(Boolean))];
  const getSubCategoriesFor = (mt, cat) => [...new Set(products.filter((p) => (!mt || p.materialType === mt) && (!cat || p.category === cat)).map((p) => p.subCategory).filter(Boolean))];
  const getFilteredProductsForGroup = (g) => products.filter((p) =>
    ...
    <div style={{ background: T.pageBg, minHeight: "100vh", padding: "20px", transition: "background-color 0.3s ease" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {filteredChallans.map((ch) => {
          ...
          const gstEntry = (ch.charges || []).find((c) => c.type === "gst");
          const gstRate = parseFloat(ch.gstRate || gstEntry?.quantity || 0);
          const gstAmt = parseFloat(ch.gstAmount || gstEntry?.amount || 0);
          return (
            <div key={ch.challanNo} className="kt-card" style={{ padding: 16 }}>
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                <div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    {gstRate > 0 && (<span className="status-pill" style={{ background: T.infoBg, color: T.infoColor }}>GST {gstRate}%</span>)}
                  </div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn-white" style={{ padding: "5px 12px", fontSize: 12 }}
                    onClick={() => openPDFView(getChallanPrintHTML(ch, ch, ch.hidePrice, regularCharges, gstRate, gstAmt))}>
                    <Eye size={12} /> View
                  </button>
                  <button className="btn-amber" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => handleEditChallan(ch)}>✏️ Edit</button>
                  <button className="btn-maroon" style={{ padding: "5px 12px", fontSize: 12 }}
                    onClick={() => openPDFPrint(getChallanPrintHTML(ch, ch, ch.hidePrice, regularCharges, gstRate, gstAmt))}>
                    <Printer size={12} /> Print
                  </button>
                          ...
                          <div key={`item-${item.uid}`} className="item-subrow">
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                ...
                                {(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.areaPerPiece && (
                              ...
                              <div style={{ display: "flex", gap: 8 }}>
                                {item.product && !item.skuCode && (
                                  <button className="icon-btn" onClick={() => handleSaveToSheet(item, group)} disabled={savingToSheet === item.uid}
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
                              ...
                              {(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.unit === "SQFT" && (
                                <div className="length-group" style={{ gridColumn: "span 2", flexWrap: 'wrap' }}>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="Ft" value={item.customLength} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customLength", e.target.value)} style={{ width: 55 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>ft</span>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="In" value={item.customLengthIn || ''} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customLengthIn", e.target.value)} style={{ width: 50 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>in ×</span>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="Ft" value={item.customWidth} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customWidth", e.target.value)} style={{ width: 55 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>ft</span>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="In" value={item.customWidthIn || ''} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customWidthIn", e.target.value)} style={{ width: 50 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>in</span>
                                  {(parseFloat(item.customLength) > 0 || parseFloat(item.customWidth) > 0) && (
                                    <span style={{ marginLeft: 8, padding: "2px 8px", background: T.cream, color: T.maroon, borderRadius: 6, fontSize: 11, fontWeight: 600 }}>
                                      = {((parseFloat(item.customLength || 0) + parseFloat(item.customLengthIn || 0) / 12) * (parseFloat(item.customWidth || 0) + parseFloat(item.customWidthIn || 0) / 12)).toFixed(2)} sqft/pc
                                    </span>
                                  )}
                                </div>
                              )}
                            ...
                            {item.product && !challanForm.hidePrice && (
                              <div className="calc-display">
                                <span style={{ fontWeight: 600 }}>Total: {item.calculatedQty.toFixed(3)} {item.unit}</span>
                                {" • "}
                ...
                {charges.map((ch, idx) => (
                  <div key={`charge-${ch.uid}`} style={{ padding: 12, border: `1px solid ${T.borderSoft}`, borderRadius: 10, marginBottom: 8, background: T.accent }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8 }}>
                      ...
                      {ch.unit !== "Lump Sum" && (<input className="kt-input" type="number" placeholder="Quantity" value={ch.quantity} onChange={(e) => updateCharge(ch.uid, "quantity", e.target.value)} />)}
              ...
              {!challanForm.hidePrice && (
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <div className="total-box" style={{ width: 320 }}>
                    ...
                    {challanForm.gstRate > 0 && (
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, color: T.textDark }}>
                        <span>GST ({challanForm.gstRate}%)</span><span>₹{gstAmount.toLocaleString()}</span>
                      </div>
                    )}
                    <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: 18, borderTop: `2px solid ${T.maroon}`, paddingTop: 8, marginTop: 8, color: T.maroon }}>
                      <span>Grand Total</span><span>₹{challanTotal.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
