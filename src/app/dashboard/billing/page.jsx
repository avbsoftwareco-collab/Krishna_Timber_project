...
//   //     console.log('Sending to sheet:', validItems.map(i => ({ product: i.product, isCharge: i.isCharge })));
...
//     // ✅ SIRF VALID ITEMS BHEJ RAHE HAIN (Charges nahi)
//     // Yeh products/material ko sheet me save karega
//     const syncRes = await syncMissingProductsToSheet(validItems);
...
//     // ✅ CHARGES ALAG SE HANDLE HO RAHE HAIN
//     // Yeh sheet me nahi jayenge, sirf order mein save honge
//     const validCharges = orderCharges
//       .filter(c => c.chargeName && c.amount > 0)
//       .map(ch => ({
//         ...ch,
...
//     // Items + Charges dono order mein save honge
//     const allItems = [...validItems, ...validCharges];
...
function SearchableSelect({
  ...
  const getDisplayFromValue = (val) => {
    if (!val) return '';
    const match = options.find(o => getVal(o) === val);
    if (match) return getDisp(match);
    if (allowCustom) return val;
    return '';
  };

  // Sync inputValue when value changes (e.g., from parent)
  useEffect(() => {
    const display = getDisplayFromValue(value);
...
const getChallanPrintHTML = (order, challan, hidePrice = false) => {
  ...
  const allRows = [...itemRows, ...chargeRows];
  const totalRows = Math.max(0, 12 - allRows.length);
  const emptyRows = Array(totalRows).fill(`<tr class="erow"><tr><td></td><td>${!hidePrice ? '<td><td><td></td>' : '<td>'}</tr>`);
...
export default function OrderChallanBilling() {
  ...
  // Manual add to sheet button handler
  const handleAddToSheet = async (group, item) => {
    ...
    try {
      ...
      if (res.success && res.data && res.data.length) {
        ...
        // Update item with new skuCode and details
  ...
  // Submit order - NO auto-sync to sheet anymore
  const handleSubmitOrder = async () => {
    if (!orderForm.customerName || getAllOrderItems().filter(i => i.product).length === 0) {
      setError('Customer name aur items required');
      return;
