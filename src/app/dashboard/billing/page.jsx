...
//     !/door/i.test(text) &&              // ✅ Door exclude karo
...
//   // ✅ 6. SHEET / DOOR CHECK (Teakwood Door, WPC Door, MDF, Ply etc.)
//   // Ye check Timber Wood se PEHLE hona chahiye
//   if (item.isSheet) {
//     if (item.unit === "SQFT") {
//       const customL = parseFloat(item.customLength || 0);
//       const customW = parseFloat(item.customWidth || 0);
...
//   // 7. Timber Wood (solid pieces - jo sheet/door nahi hai)
...
//   // 8. Default fallback (Hardware, Pcs etc.)
//   return {
//     calculatedQty: qty,
//     amount: Math.round(qty * rate * 100) / 100,
//   };
...
// .page-content{height:288mm!important;min-height:288mm!important;max-height:288mm!important;display:flex!important;flex-direction:column!important;overflow:hidden!important;}
...
//           {item.quantity || 0} pcs × {effectiveArea.toFixed(2)} sqft ={" "}
//           {item.calculatedQty.toFixed(2)} SQFT
//         </span>
//       );
//     })()}
//     {" • "}
//     <span style={{ fontWeight: "bold", color: T.maroon }}>
...
//         </span>
//       </>
//     ) : item.isSheet && (item.unit === "Per Piece" || item.unit === "Pcs") ? (
//       <>
//         <span style={{ fontWeight: 600 }}>
//           Qty: {item.calculatedQty} pcs
//         </span>
...
// const WOOD_UNIT_OPTIONS = ["CFT", "RFT", "SQFT", "PCS"];
// const HARDWARE_UNIT_OPTIONS = ["PCS", "Pkt", "Set", "Kg"];
// const CUSTOM_UNIT_OPTIONS = ["PCS", "CFT", "RFT", "SQFT", "Kg", "Meter", "Box", "Set", "Pkt", "Dozen"];
// const SHEET_UNIT_OPTIONS = ["SQFT", "Per Piece"];
// const CHARGE_UNIT_OPTIONS = ["CFT", "RFT", "SQFT", "PCS", "Lump Sum"];
...
//       };
//     }
//     if (item.unit === "Per Piece" || item.unit === "Pcs" || item.unit === "PCS") {
//       return { calculatedQty: qty, amount: Math.round(qty * rate * 100) / 100 };
//     }
//   }
...
// function getChallanPrintHTML(order, challan, hidePrice = false, chargesList = [], gstRate = 0, gstAmount = 0) {
//   const regularItems = (challan.items || []).filter((it) => !it.isCharge);
...
//     const ld = it.lengthDisplay || "";
//     if (ld && ld !== "0'-0\"" && ld !== "'-\"" && ld !== "-") details.push(ld);
//     if (details.length) descText += ` <span class="item-detail" style="display:inline;">${details.join(" · ")}</span>`;
//     return `<tr><td class="c">${sno}</td><td class="tl">${descText}</td>${!hidePrice ? `<td class="r">${qtyWithUnit}</td><td class="r">₹${parseFloat(it.rate || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td><td class="r"><strong>₹${parseFloat(it.amount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</strong></td>` : `<td class="r">${qtyWithUnit}</td>`}</tr>`;
//   });
...
//     ${gstRate > 0 ? `<div class="ktp-total-row"><span class="ktp-total-label">Sub Total</span><span class="ktp-total-val">₹${subTotalWithCharges.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
...
//     ${roundUpVal !== 0 ? `<div class="ktp-total-row"><span class="ktp-total-label">Round ${roundUpVal > 0 ? 'Up' : 'Off'}</span><span class="ktp-total-val">${roundUpVal > 0 ? '+' : ''}₹${roundUpVal.toFixed(2)}</span></div>` : ""}
//     <div class="ktp-total-row grand"><span class="ktp-total-label">GRAND TOTAL</span><span class="ktp-total-val">₹${challanTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>
//     <div class="ktp-eoe-line">E. &amp; O.E.</div>`;
//   }
...
//     vehicleNo: "", challanDate: new Date().toISOString().split("T")[0],
//     gstRate: 0, notes: "", poNumber: "", gstCustomerName: "",
//     hidePrice: false, roundUp: "",
//   });
//   const [itemGroups, setItemGroups] = useState([createEmptyGroup()]);
//   const [charges, setCharges] = useState([]);
...
//       return pChallan === challanNo;
//     });
//     const totalPaid = challanPayments.reduce((s, p) => s + parseFloat(p.amount || 0), 0);
//     const due = challanTotal - totalPaid;
//     if (totalPaid <= 0) return { status: 'unpaid', paid: 0, due: challanTotal, total: challanTotal };
//     else if (due <= 0.5) return { status: 'paid', paid: totalPaid, due: 0, total: challanTotal };
//     else return { status: 'partial', paid: totalPaid, due: due, total: challanTotal };
//   };
...
//   function createEmptyGroup() {
//     return {
//       groupId: uid(), filterMaterialType: "", filterCategory: "",
//       filterSubCategory: "", items: [createEmptyItem()],
//     };
//   }

//   function createEmptyCharge() {
//     return {
//       uid: uid(), chargeType: "", chargeName: "", chargeDescription: "",
//       unit: "Per Piece", quantity: "", rate: "", amount: 0,
//     };
//   }

//   const getAllItems = () =>
//     itemGroups.flatMap((g) => g.items.map((i) => ({
//       ...i, filterMaterialType: g.filterMaterialType,
//       filterCategory: g.filterCategory, filterSubCategory: g.filterSubCategory,
//     })));
...
//                     u.width = dims.widthFeet;
//                     u.thickness = dims.thickness;
//                     u.size = dims.isInches ? `${dims.widthOrig}"×${dims.heightOrig}"` : `${dims.widthFeet}'×${dims.heightFeet}'`;
//                     u.customLength = dims.widthFeet;
//                     u.customWidth = dims.heightFeet;
//                   }
//                   u.lengthFeet = "";
...
//         .filter((i) => i.product && (i.quantity || i.calculatedQty))
//         .map((it) => ({
//           product: it.product, unit: it.unit, quantity: it.quantity,
//           rate: it.rate, amount: it.amount, calculatedQty: it.calculatedQty,
//           size: it.size,
//           lengthDisplay: (it.isWood || isTimberWood(it) || isWoodenCladding(it) || isWoodenBaluster(it)) ? `${it.lengthFeet || 0}'-${it.lengthInches || 0}"` : "",
//           specification: it.specification || "", isSheet: it.isSheet,
//           areaPerPiece: it.areaPerPiece,
//           pieces: parseFloat(it.quantity || 0),
//           sentQty: parseFloat(it.quantity || 0),
//           orderedQty: it.calculatedQty,
...
//         .filter((c) => c.chargeName && c.amount > 0)
//         .map((ch) => ({
//           name: ch.chargeName, type: ch.chargeType || "custom",
//           unit: ch.unit || "Per Piece", quantity: ch.quantity || "",
//           rate: ch.rate || "", amount: ch.amount,
//         }));

//       if (challanForm.gstRate > 0 && gstAmount > 0) {
//         validCharges.push({
//           name: `GST (${challanForm.gstRate}%)`, type: "gst",
//           unit: "Percentage", quantity: challanForm.gstRate,
//           rate: challanForm.gstRate, amount: gstAmount,
//         });
//       }
...
//           challanDate: challanForm.challanDate,
//           deliveryNote: challanForm.notes,
//           challanTotal: finalTotal, status: "Delivered",
//           hidePrice: challanForm.hidePrice,
//           gstRate: challanForm.gstRate, gstAmount,
//           subtotal: itemsSubtotal, chargesTotal: chargesSubtotal,
//           roundUp: finalRoundUp,
//         },
//         items: validItems,
//         charges: validCharges,
...
//       let res;
//       if (editingChallan) res = await apiPut("/api/billing-backend/challans", payload);
//       else res = await apiPost("/api/billing-backend/challans", payload);
...
//           category: group.filterCategory || item.category || "Custom",
//           subCategory: group.filterSubCategory || item.subCategory || "",
//           materialName: item.product, unit: item.unit || "PCS",
//         },
//       });
//       if (res.success) {
//         const pR = await apiGet("/api/dropdown-data");
...
//     } catch (err) { setError(err.message); }
//     finally { setSavingToSheet(null); }
//   };

//   // ✅ FIXED: Edit me sab data properly fetch karega
//   const handleEditChallan = (ch) => {
//     setChallanForm({
//       customerName: ch.customerName || "",
//       customerPhone: ch.customerPhone || "",
...
//       gstCustomerName: ch.gstCustomerName || "",
//       hidePrice: ch.hidePrice || false,
//       roundUp: ch.roundUp ? ch.roundUp.toString() : "",
//     });

//     const savedItems = (ch.items || []).filter(it => !it.isCharge);
//     if (savedItems.length > 0) {
//      const groupItems = savedItems.map(it => {
//   // ✅ Product name ke basis pe detect karo (edit me materialType/category save nahi hote)
//   const tempItem = {
//     product: it.product,
//     materialType: it.materialType || it.product || "",
//     category: it.category || it.product || "",
//   };
...
//             if (inchMatch) {
//               parsedWidth = parseFloat(inchMatch[1]) || 0;
//               parsedThickness = parseFloat(inchMatch[2]) || 0;
//             }
//           }
//         }

//         // Fallback to parsed dimensions
...
//         let areaPerPc = null;
//         if (customL && customW) {
//           areaPerPc = parseFloat(customL) * parseFloat(customW);
//         } else if (sheetDims) {
//           areaPerPc = sheetDims.areaPerPiece;
//         } else if (solidDims) {
//           areaPerPc = solidDims.areaPerPiece;
//         }
...
//           areaPerPiece: areaPerPc,
//           customLength: customL,
//           customWidth: customW,
//           lengthDisplay: it.lengthDisplay || "",
//           specification: it.specification || "",
//           dfSectionWidth: doorDims?.sectionWidth || "",
//           dfSectionThickness: doorDims?.sectionThickness || "",
//           dfHeightFt: doorDims?.heightFt || "",
//           dfHeightIn: doorDims?.heightIn || "",
//           dfWidthFt: doorDims?.widthFt || "",
//           dfWidthIn: doorDims?.widthIn || "",
...
//         <h2 style={{ fontSize: 24, fontWeight: "bold", color: T.maroon, margin: 0 }}>Delivery Challans</h2>
//         <button className="btn-maroon" onClick={() => { setEditingChallan(null); resetForm(); setShowChallanForm(true); }}>
//           <Plus size={16} /> New Challan
//         </button>
//       </div>
...
//           <input className="kt-input" style={{ paddingLeft: 36 }} placeholder="Search challan..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value.toLowerCase())} />
//         </div>
...
//                         background: payStatus.status === 'paid' ? (darkMode ? '#052e16' : '#dcfce7') : payStatus.status === 'partial' ? (darkMode ? '#451a03' : '#fef3c7') : (darkMode ? '#450a0a' : '#fef2f2'),
//                         color: payStatus.status === 'paid' ? (darkMode ? '#4ade80' : '#166534') : payStatus.status === 'partial' ? (darkMode ? '#fbbf24' : '#92400e') : (darkMode ? '#fca5a5' : '#dc2626'),
//                         border: `1px solid ${payStatus.status === 'paid' ? (darkMode ? '#166534' : '#bbf7d0') : payStatus.status === 'partial' ? (darkMode ? '#92400e' : '#fde68a') : (darkMode ? '#7f1d1d' : '#fecaca')}`,
//                         fontWeight: 700, fontSize: 11,
//                       }}>
//                         <span className="status-dot" style={{ background: payStatus.status === 'paid' ? '#22c55e' : payStatus.status === 'partial' ? '#f59e0b' : '#ef4444' }} />
//                         {payStatus.status === 'paid' ? '✓ PAID' : payStatus.status === 'partial' ? '◑ PARTIAL' : '✗ UNPAID'}
//                       </span>
//                     )}
//                     {ch.hidePrice && (
...
//                     )}
//                     {gstRate > 0 && (
//                       <span className="status-pill" style={{ background: T.infoBg, color: T.infoColor }}>GST {gstRate}%</span>
//                     )}
//                   </div>
...
//                     {payStatus && payStatus.status === 'partial' && (<span style={{ color: '#d97706', fontWeight: 600 }}> • Due: ₹{payStatus.due.toFixed(0)}</span>)}
//                     {payStatus && payStatus.status === 'unpaid' && (<span style={{ color: '#dc2626', fontWeight: 600 }}> • Unpaid</span>)}
//                     {payStatus && payStatus.status === 'paid' && (<span style={{ color: '#166534', fontWeight: 600 }}> • Fully Paid ✓</span>)}
//                   </p>
//                 </div>
//                 <div style={{ display: "flex", gap: 8 }}>
//                   <button className="btn-white" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => {
//                     openPDFView(getChallanPrintHTML({ ...ch, roundUp: ch.roundUp || 0 }, ch, ch.hidePrice, regularCharges, gstRate, gstAmount));
//                   }}>
//                     <Eye size={12} /> View
//                   </button>
//                   <button className="btn-amber" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => handleEditChallan(ch)}>
//                     ✏️ Edit
//                   </button>
//                   <button className="btn-maroon" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => {
//                     openPDFPrint(getChallanPrintHTML({ ...ch, roundUp: ch.roundUp || 0 }, ch, ch.hidePrice, regularCharges, gstRate, gstAmount));
//                   }}>
//                     <Printer size={12} /> Print
//                   </button>
//                   <button className="icon-btn" style={{ color: T.errorColor }} onClick={() => handleDeleteChallan(ch.challanNo)}>
//                     <Trash2 size={14} />
//                   </button>
//                 </div>
//               </div>
...
//                 {editingChallan ? `Edit Challan - ${editingChallan.challanNo}` : "New Delivery Challan"}
//               </h3>
//               <button className="icon-btn" onClick={() => { setShowChallanForm(false); setEditingChallan(null); resetForm(); }}>
//                 <X size={24} />
//               </button>
//             </div>
...
//                   <input className="kt-input" placeholder="Customer Name *" value={challanForm.customerName} onChange={(e) => setChallanForm({ ...challanForm, customerName: e.target.value })} />
//                   <input className="kt-input" placeholder="Phone" value={challanForm.customerPhone} onChange={(e) => setChallanForm({ ...challanForm, customerPhone: e.target.value })} />
//                   <input className="kt-input" placeholder="Vehicle No." value={challanForm.vehicleNo} onChange={(e) => setChallanForm({ ...challanForm, vehicleNo: e.target.value })} />
//                   <input type="date" className="kt-input" value={challanForm.challanDate} onChange={(e) => setChallanForm({ ...challanForm, challanDate: e.target.value })} />
//                   <select className="kt-input" value={challanForm.gstRate} onChange={(e) => setChallanForm({ ...challanForm, gstRate: parseFloat(e.target.value) })}>
//                     {GST_OPTIONS.map((o) => (<option key={`gst-opt-${o.value}`} value={o.value}>{o.label}</option>))}
//                   </select>
//                   <input className="kt-input" placeholder="PO Number" value={challanForm.poNumber} onChange={(e) => setChallanForm({ ...challanForm, poNumber: e.target.value })} />
//                   <input className="kt-input" placeholder="GST Customer Name" value={challanForm.gstCustomerName} onChange={(e) => setChallanForm({ ...challanForm, gstCustomerName: e.target.value })} />
//                   <textarea className="kt-input" rows="2" placeholder="Address" value={challanForm.customerAddress} onChange={(e) => setChallanForm({ ...challanForm, customerAddress: e.target.value })} style={{ gridColumn: "span 2" }} />
//                 </div>
//               </div>

//               <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 20, color: T.textDark }}>
//                 <input type="checkbox" checked={challanForm.hidePrice} onChange={(e) => setChallanForm({ ...challanForm, hidePrice: e.target.checked })} />
//                 <EyeOff size={18} /> Hide Price on Challan
//               </label>
...
//                         </div>
//                         {itemGroups.length > 1 && (
//                           <button className="icon-btn" onClick={() => removeGroup(group.groupId)}><Trash2 size={16} /></button>
//                         )}
//                       </div>
...
//                                 {item.isWood && !item.isSheet && !isTimberWood(item) && !isWoodenCladding(item) && !isWoodenBaluster(item) && (<span className="unit-badge wood-badge">Wood</span>)}
...
//                                 {(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.areaPerPiece && (<span style={{ fontSize: 10, color: T.textMuted, background: T.cream, padding: "2px 6px", borderRadius: 6 }}>{item.areaPerPiece.toFixed(2)} sqft/pc</span>)}
//                               </div>
//                               <div style={{ display: "flex", gap: 8 }}>
//                                 {item.product && !item.skuCode && (
//                                   <button className="icon-btn" onClick={() => handleSaveToSheet(item, group)} disabled={savingToSheet === item.uid} title="Save this product to Dropdown sheet"
//                                     style={{ color: T.successColor, background: T.successBg, padding: "4px 10px", fontSize: 11, fontWeight: 600, width: "auto", gap: 4, opacity: savingToSheet === item.uid ? 0.6 : 1, cursor: savingToSheet === item.uid ? "wait" : "pointer" }}>
//                                     {savingToSheet === item.uid ? (<><Loader2 size={12} className="animate-spin" /> Saving...</>) : (<>💾 Save</>)}
//                                   </button>
//                                 )}
...
//                                     {DOOR_FRAME_UNIT_OPTIONS.map((u) => (<option key={`df-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : isSolidSurface(item) ? (
//                                   <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {SOLID_SURFACE_UNIT_OPTIONS.map((u) => (<option key={`ss-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : isCeilingPlank(item) ? (
//                                   <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {CEILING_PLANK_UNIT_OPTIONS.map((u) => (<option key={`cp-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : isWoodenBaluster(item) ? (
...
//                                     {WOODEN_BALUSTER_UNIT_OPTIONS.map((u) => (<option key={`wb-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : isWoodenCladding(item) ? (
//                                   <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {WOODEN_CLADDING_UNIT_OPTIONS.map((u) => (<option key={`wc-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : isTimberWood(item) ? (
//                                   <select className="kt-input" value={item.unit || "CFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {TIMBER_UNIT_OPTIONS.map((u) => (<option key={`tw-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : item.isSheet ? (
//                                   <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
...
//                                   </select>
//                                 ) : /kitchen/i.test(item.materialType || item.category || group.filterMaterialType || group.filterCategory || "") ? (
//                                   <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {KITCHEN_UNIT_OPTIONS.map((u) => (<option key={`kit-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : /louver/i.test(item.materialType || item.category || group.filterMaterialType || group.filterCategory || "") ? (
//                                   <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {LOUVER_UNIT_OPTIONS.map((u) => (<option key={`lou-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : /laminate/i.test(item.materialType || item.category || group.filterMaterialType || group.filterCategory || "") ? (
//                                   <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {LAMINATE_UNIT_OPTIONS.map((u) => (<option key={`lam-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : item.skuCode ? (
//                                   <select className="kt-input" value={item.unit || "PCS"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {[...new Set([item.unit, ...HARDWARE_UNIT_OPTIONS].filter(Boolean))].map((u) => (<option key={`hw-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 ) : (
//                                   <select className="kt-input" value={item.unit || "PCS"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
//                                     {[...new Set([item.unit, ...CUSTOM_UNIT_OPTIONS].filter(Boolean))].map((u) => (<option key={`cu-u-${u}`} value={u}>{u}</option>))}
//                                   </select>
//                                 )}
//                               </div>
...
//                                 <div style={{ gridColumn: '1 / -1', display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', padding: '8px 0' }}>
//                                   <span style={{ fontSize: 13, color: T.textMuted }}>V.Qty</span>
//                                   <input type="number" className="kt-input" placeholder="V.Qty" value={item.dfVerticalQty} onChange={e => updateGroupItem(group.groupId, item.uid, 'dfVerticalQty', e.target.value)} style={{ width: 70 }} />
//                                   <span style={{ fontSize: 13, color: T.textMuted }}>H.Qty</span>
//                                   <input type="number" className="kt-input" placeholder="H.Qty" value={item.dfHorizontalQty} onChange={e => updateGroupItem(group.groupId, item.uid, 'dfHorizontalQty', e.target.value)} style={{ width: 70 }} />
//                                   <span style={{ color: T.textMuted, fontSize: 16, fontWeight: 'bold' }}>×</span>
...
//                                 {isDoorFrame(item) ? (
//                                   <>
//                                     <span style={{ fontWeight: 600 }}>Section: {item.dfSectionWidth || "?"}" × {item.dfSectionThickness || "?"}" &nbsp;|&nbsp; Height: {item.dfHeightFt || 0}'{item.dfHeightIn || 0}" &nbsp;|&nbsp; Door Width: {item.dfWidthFt || 0}'{item.dfWidthIn || 0}" &nbsp;|&nbsp; {item.quantity || 0} Frames = {item.calculatedQty.toFixed(3)} CFT</span>
...
//                                       const customL = parseFloat(item.customLength || 0);
//                                       const customW = parseFloat(item.customWidth || 0);
//                                       const effectiveArea = customL > 0 && customW > 0 ? customL * customW : item.areaPerPiece || 0;
//                                       return (<span style={{ fontWeight: 600 }}>{item.quantity || 0} pcs × {effectiveArea.toFixed(2)} sqft = {item.calculatedQty.toFixed(2)} SQFT</span>);
//                                     })()}
//                                     {" • "}<span style={{ fontWeight: "bold", color: T.maroon }}>₹{item.amount.toLocaleString()}</span>
//                                   </>
//                                 ) : isCeilingPlank(item) ? (
...
//                                         return `${item.quantity || 0} pcs × ${effectiveArea.toFixed(2)} sqft = ${item.calculatedQty.toFixed(2)} SQFT`;
//                                       })()}
//                                       {item.unit === "RFT" && `Total RFT: ${item.calculatedQty.toFixed(2)}`}
//                                       {item.unit === "Per Piece" && `Qty: ${item.calculatedQty} pcs`}
//                                     </span>
//                                     {" • "}<span style={{ fontWeight: "bold", color: T.maroon }}>₹{item.amount.toLocaleString()}</span>
//                                   </>
//                                 ) : isWoodenBaluster(item) ? (
//                                   <>
//                                     <span style={{ fontWeight: 600 }}>
//                                       {item.unit === "CFT" && `Total CFT: ${item.calculatedQty.toFixed(3)}`}
//                                       {item.unit === "Per Piece" && `Qty: ${item.calculatedQty} pcs`}
//                                     </span>
//                                     {" • "}<span style={{ fontWeight: "bold", color: T.maroon }}>₹{item.amount.toLocaleString()}</span>
//                                   </>
...
//                                     {" • "}<span style={{ fontWeight: "bold", color: T.maroon }}>₹{item.amount.toLocaleString()}</span>
//                                   </>
//                                 ) : item.isSheet && (item.unit === "Per Piece" || item.unit === "Pcs" || item.unit === "PCS") ? (
//                                   <>
//                                     <span style={{ fontWeight: 600 }}>Qty: {item.calculatedQty} pcs</span>
//                                     {" • "}<span style={{ fontWeight: "bold", color: T.maroon }}>₹{item.amount.toLocaleString()}</span>
//                                   </>
//                                 ) : isTimberWood(item) ? (
//                                   <>
...
//                   <button className="btn-amber" style={{ padding: "6px 14px", fontSize: 13 }} onClick={addCharge}>
//                     <Plus size={14} /> Add Charge
//                   </button>
//                 </div>
//                 {charges.map((ch, idx) => (
...
//                       </div>
//                     )}
//                     <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, paddingTop: 8, borderTop: `1px dashed ${T.borderSoft}` }}>
//                       <span style={{ fontSize: 13, color: T.textDark, fontWeight: 500 }}>Round Off</span>
//                       <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
//                         <button onClick={() => { const diff = +(Math.round(calculatedTotal) - calculatedTotal).toFixed(2); setChallanForm({ ...challanForm, roundUp: diff.toString() }); }}
//                           style={{ padding: "3px 8px", fontSize: 10, borderRadius: 6, border: `1px solid ${T.borderSoft}`, background: T.creamDark, color: T.textDark, cursor: "pointer", fontWeight: 600 }} title="Nearest ₹1">₹1</button>
//                         <button onClick={() => { const diff = +(Math.round(calculatedTotal / 10) * 10 - calculatedTotal).toFixed(2); const clamped = Math.max(-9.99, Math.min(9.99, diff)); setChallanForm({ ...challanForm, roundUp: clamped.toString() }); }}
//                           style={{ padding: "3px 8px", fontSize: 10, borderRadius: 6, border: `1px solid ${T.borderSoft}`, background: T.creamDark, color: T.textDark, cursor: "pointer", fontWeight: 600 }} title="Nearest ₹10 (max ±9)">₹10</button>
//                         <button onClick={() => setChallanForm({ ...challanForm, roundUp: "" })}
//                           style={{ padding: "3px 6px", fontSize: 10, borderRadius: 6, border: `1px solid ${T.borderSoft}`, background: T.creamDark, color: T.errorColor, cursor: "pointer", fontWeight: 600 }} title="Clear">✕</button>
//                         <input type="number" step="0.01" min="-9.99" max="9.99" placeholder="0.00" value={challanForm.roundUp}
//                           onChange={e => {
//                             let val = parseFloat(e.target.value || 0);
//                             if (val > 9.99) val = 9.99;
//                             if (val < -9.99) val = -9.99;
//                             setChallanForm({ ...challanForm, roundUp: e.target.value === "" ? "" : val.toString() });
//                           }}
//                           style={{ width: 72, padding: "4px 6px", border: `1.5px solid ${roundUpValue > 0 ? T.successColor : roundUpValue < 0 ? T.errorColor : T.borderSoft}`, borderRadius: 8, fontSize: 13, background: T.inputBg, color: T.textDark, outline: "none", textAlign: "right" }}
//                         />
//                       </div>
//                     </div>
//                     {roundUpValue !== 0 && (
//                       <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12, color: roundUpValue > 0 ? T.successColor : T.errorColor }}>
//                         <span>{roundUpValue > 0 ? "Round Up (+)" : "Round Down (−)"}</span>
//                         <span style={{ fontWeight: 600 }}>{roundUpValue > 0 ? "+" : ""}₹{roundUpValue.toFixed(2)}</span>
//                       </div>
//                     )}
//                     <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: 18, borderTop: `2px solid ${T.maroon}`, paddingTop: 8, marginTop: 4, color: T.maroon }}>
//                       <span>Grand Total</span>
//                       <span>₹{challanTotal.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
//                     </div>
//                   </div>
...
//               <button className="btn-white" onClick={() => { setShowChallanForm(false); setEditingChallan(null); resetForm(); }}>Cancel</button>
