...
table.items th:last-child{border-right:none}
table.items th.tl{text-align:left}
table.items tbody{height:auto;min-height:200px;}
...
table.items .erow td{height:38px;border-right:1px solid #000}
table.items .erow td:last-child{border-right:none}
/* ✅ Fixed spacer row for screen */
table.items .spacer-row td {
  height: auto !important;
  border-right: 1px solid #000;
  border-bottom: none !important;
}
table.items .spacer-row td:last-child {
...
function getGoodsReturnPrintHTML(returnData, returnItemsList, challan, order) {
  ...
  let sno = 0;
  const itemRowsArr = returnItemsList.map(it => {
    sno++;
    let qtyVal = formatQty(it.returnQty);
    let qtyWithUnit = it.unit ? `${qtyVal} ${it.unit}` : qtyVal;
    if (it.returnPcs && parseFloat(it.returnPcs) !== parseFloat(it.returnQty)) {
  ...
  if (!hidePrice) {
    footerRightLast = `
      ${reasonObj ? `<div class="ktp-reason-box">📋 Reason: ${reasonObj.icon} ${reasonObj.label}</div>` : ''}
      <div class="ktp-total-row"><span class="ktp-total-label">Items Returned</span><span class="ktp-total-val">${returnItemsList.length}</span></div>
      <div class="ktp-total-row grand"><span class="ktp-total-label">CREDIT TOTAL</span><span class="ktp-total-val">-₹${returnTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span></div>
      <div class="ktp-eoe-line">E. &amp; O.E.</div>
    `;
  } else {
    footerRightLast = `
  ...
  const footerRightContinued = `<div class="ktp-no-price-box"><div>Continued on next page...</div></div>`;

  const customerInfoHTML = `
    <div class="ktp-compact-row">
      <span class="ktp-info-title-box">RETURNED BY (CUSTOMER DETAILS)</span>
      <div class="ktp-compact-field"><span class="ktp-compact-label">Phone No.:</span><span class="ktp-compact-value">${order?.customerPhone || challan?.customerPhone || ''}</span></div>
    </div>
    <div class="ktp-compact-row">
      <div class="ktp-compact-field flex-name"><span class="ktp-compact-label">Name:</span><span class="ktp-compact-value">${returnData.customerName || ''}</span></div>
      <div class="ktp-compact-field"><span class="ktp-compact-label">GST No.:</span><span class="ktp-compact-value">${challan?.gstCustomerName || ''}</span></div>
    </div>
    <div class="ktp-compact-row">
      <div class="ktp-compact-field"><span class="ktp-compact-label">Address:</span><span class="ktp-compact-value">${order?.customerAddress || challan?.customerAddress || ''}</span></div>
    </div>
  `;
