...
// // ============================================================
// // ✅ MAIN COMPONENT - QuotationOnlyBilling
// // ============================================================
// export default function QuotationOnlyBilling() {
...
//   const [lastQuotationHTML, setLastQuotationHTML] = useState("");
//   const [lastQuotationNo, setLastQuotationNo] = useState("");
...
//     customerPhone: "",
//     customerAddress: "",
//     vehicleNo: "",          // "Valid Till" ke liye use ho raha hai
//     quotationDate: new Date().toISOString().split("T")[0],
//     gstRate: 0,
//     notes: "",
//     poNumber: "",           // "Ref No." ke liye
//     gstCustomerName: "",
//     hidePrice: false,
//   });
...
//   const gstAmount = quotationForm.gstRate > 0
//     ? (itemsSubtotal + chargesSubtotal) * (quotationForm.gstRate / 100)
//     : 0;
//   const quotationTotal = itemsSubtotal + chargesSubtotal + gstAmount;

//   const fetchData = useCallback(async () => {
//     setLoading(true);
//     try {
//       const [qR, pR] = await Promise.all([
//         apiGet("/api/billing-backend/quotations"),
//         apiGet("/api/dropdown-data"),
//       ]);
//       setQuotations(qR.success ? qR.data : []);
...
//     const prefix = `QT-${y}-`;
//     const max = quotations
//       .filter((q) => q.quotationNo?.startsWith(prefix))
//       .reduce(
//         (m, q) => Math.max(m, parseInt(q.quotationNo.replace(prefix, "")) || 0),
//         0,
//       );
...
//       setItemGroups((prev) => prev.filter((g) => g.groupId !== gid));
//   };

//   const addNewGroup = () =>
//     setItemGroups((prev) => [...prev, createEmptyGroup()]);
...
//             if (['dfSectionWidth', 'dfSectionThickness', 'dfHeightFt', 'dfHeightIn',
//                  'dfWidthFt', 'dfWidthIn', 'dfVerticalQty', 'dfHorizontalQty'].includes(field)) {
//               u[field] = val;
//             }
...
//   const addCharge = () =>
//     setCharges((prev) => [...prev, createEmptyCharge()]);
//   const removeCharge = (cuid) =>
//     setCharges((prev) => prev.filter((c) => c.uid !== cuid));
...
//       getAllItems().filter((i) => i.product).length === 0
//     ) {
...
//     setError(null);
//     try {
//       const quotationNo = editingQuotation
...
//       if (quotationForm.gstRate > 0 && gstAmount > 0) {
//         validCharges.push({
//           name: `GST (${quotationForm.gstRate}%)`,
//           type: "gst",
//           unit: "Percentage",
//           quantity: quotationForm.gstRate,
//           rate: quotationForm.gstRate,
//           amount: gstAmount,
//         });
//       }
...
//           vehicleNo: quotationForm.vehicleNo,
//           poNumber: quotationForm.poNumber,
//           gstCustomerName: quotationForm.gstCustomerName,
//           quotationDate: quotationForm.quotationDate,
//           deliveryNote: quotationForm.notes,
//           quotationTotal,
//           status: "Pending",
//           hidePrice: quotationForm.hidePrice,
...
//       let res;
//       if (editingQuotation) {
//         res = await apiPut("/api/billing-backend/quotations", payload);
//       } else {
//         res = await apiPost("/api/billing-backend/quotations", payload);
//       }

//       if (!res.success) throw new Error(res.error);

//       const html = getQuotationPrintHTML(
//         {
//           customerName: quotationForm.customerName,
//           customerPhone: quotationForm.customerPhone,
//           customerAddress: quotationForm.customerAddress,
...
//           gstCustomerName: quotationForm.gstCustomerName,
//         },
//         { ...payload.quotation, items: validItems },
//         quotationForm.hidePrice,
//         validCharges.filter((c) => c.type !== "gst"),
//         quotationForm.gstRate,
//         gstAmount,
//       );

//       setLastQuotationHTML(html);
...
//   const handleDeleteQuotation = async (quotationNo) => {
//     if (!confirm(`Delete quotation ${quotationNo}?`)) return;
//     try {
//       const res = await apiDelete("/api/billing-backend/quotations", {
//         quotationNo,
//       });
...
//   const handleEditQuotation = (qt) => {
//     setQuotationForm({
//       customerName: qt.customerName || "",
//       customerPhone: qt.customerPhone || "",
//       customerAddress: qt.customerAddress || "",
//       vehicleNo: qt.vehicleNo || "",
//       quotationDate:
//         qt.quotationDate || new Date().toISOString().split("T")[0],
//       gstRate: parseFloat(qt.gstRate) || 0,
//       notes: qt.deliveryNote || "",
//       poNumber: qt.poNumber || "",
//       gstCustomerName: qt.gstCustomerName || "",
//       hidePrice: qt.hidePrice || false,
//     });

//     const savedItems = (qt.items || []).filter((it) => !it.isCharge);
...
//     const savedCharges = (qt.charges || []).filter((c) => c.type !== "gst");
...
//     }
//     if (
//       !confirm(`"${item.product}" ko Dropdown sheet mein save karna hai?`)
//     )
//       return;
//     setSavingToSheet(item.uid);
//     try {
//       const res = await apiPost("/api/dropdown-data", {
//         product: {
//           materialType:
//             group.filterMaterialType || item.materialType || "Custom",
...
//   )
//   .sort((a, b) => {
//     const numA = parseInt(a.quotationNo?.replace(/\D/g, '') || 0);
//     const numB = parseInt(b.quotationNo?.replace(/\D/g, '') || 0);
//     return numB - numA;
//   });
...
//         .filter(
//           (p) =>
//             (!mt || p.materialType === mt) && (!cat || p.category === cat),
//         )
//         .map((p) => p.subCategory)
//         .filter(Boolean),
//     ),
...
//         .ss-option{padding:10px 12px;cursor:pointer;border-bottom:1px solid ${T.accent};color:${T.textDark}}
...
//         .length-group{display:flex;align-items:center;gap:6px}
//         .length-input{width:70px}
//       `}</style>
...
//           const regularCharges = (qt.charges || []).filter(
//             (c) => c.type !== "gst",
//           );
//           const gstEntry = (qt.charges || []).find((c) => c.type === "gst");
//           const gstRate = parseFloat(qt.gstRate || gstEntry?.quantity || 0);
//           const gstAmt = parseFloat(qt.gstAmount || gstEntry?.amount || 0);
...
//                     {(qt.items || []).length} items
//                     {!qt.hidePrice &&
//                       ` • ₹${parseFloat(qt.quotationTotal || 0).toLocaleString()}`}
//                   </p>
//                 </div>
...
//                     className="btn-amber"
//                     style={{ padding: "5px 12px", fontSize: 12 }}
//                     onClick={() => handleEditQuotation(qt)}
//                   >
//                     ✏️ Edit
//                   </button>
//                   <button
...
//                     onClick={() => handleDeleteQuotation(qt.quotationNo)}
//                   >
//                     <Trash2 size={14} />
//                   </button>
//                 </div>
...
//               >
//                 {editingQuotation
//                   ? `Edit Quotation - ${editingQuotation.quotationNo}`
//                   : "New Quotation"}
//               </h3>
//               <button
//                 className="icon-btn"
...
//                   <input
//                     className="kt-input"
//                     placeholder="Valid Till (e.g. 30 Days)"
//                     value={quotationForm.vehicleNo}
//                     onChange={(e) =>
//                       setQuotationForm({
//                         ...quotationForm,
...
//                     value={quotationForm.gstCustomerName}
//                     onChange={(e) =>
//                       setQuotationForm({
//                         ...quotationForm,
...
//                 <input
//                   type="checkbox"
//                   checked={quotationForm.hidePrice}
//                   onChange={(e) =>
//                     setQuotationForm({
//                       ...quotationForm,
...
//                   }
//                 />
//                 <EyeOff size={18} /> Hide Price on Quotation
//               </label>
...
//                                     {[
//                                       ...new Set(
//                                         [item.unit, ...CUSTOM_UNIT_OPTIONS].filter(
//                                           Boolean,
//                                         ),
//                                       ),
//                                     ].map((u) => (
...
//                                     step="0.5"
//                                     className="kt-input"
//                                     placeholder='Sec.Width (")'
//                                     value={item.dfSectionWidth}
//                                     onChange={(e) =>
//                                       updateGroupItem(
//                                         group.groupId,
//                                         item.uid,
...
//                             {/* Calc display */}
//                             {item.product && !quotationForm.hidePrice && (
//                               <div className="calc-display">
//                                 <span style={{ fontWeight: 600 }}>
//                                   {isDoorFrame(item) &&
//                                     `CFT: ${item.calculatedQty.toFixed(3)}`}
//                                   {(isTimberWood(item) ||
//                                     isWoodenCladding(item) ||
//                                     isWoodenBaluster(item)) &&
//                                     `Total ${item.unit}: ${item.calculatedQty.toFixed(3)}`}
//                                   {item.isSheet &&
//                                     item.unit === "SQFT" &&
//                                     (() => {
//                                       const effectiveArea =
//                                         parseFloat(item.customLength || 0) *
//                                           parseFloat(item.customWidth || 0) ||
//                                         item.areaPerPiece ||
//                                         0;
//                                       return `${item.quantity || 0} pcs × ${effectiveArea.toFixed(2)} sqft = ${item.calculatedQty.toFixed(2)} SQFT`;
//                                     })()}
//                                   {isSolidSurface(item) &&
//                                     `${item.quantity || 0} pcs = ${item.calculatedQty.toFixed(2)} SQFT`}
//                                   {!isDoorFrame(item) &&
//                                     !isTimberWood(item) &&
//                                     !isWoodenCladding(item) &&
//                                     !isWoodenBaluster(item) &&
...
//                         }}
//                       >
//                         {CHARGE_TYPES.find((t) => t.value === ch.chargeType)
//                           ?.icon || "📋"}{" "}
//                         #{idx + 1} {ch.chargeName || "New Charge"}
//                       </span>
//                       <button
//                         className="icon-btn"
...
//                         }}
//                       >
//                         <span>GST ({quotationForm.gstRate}%)</span>
...
//               <button
//                 className="btn-teal"
//                 onClick={() => openPDFView(lastQuotationHTML)}
//               >
//                 <Eye size={16} /> View
//               </button>
//               <button
//                 className="btn-maroon"
//                 onClick={() => openPDFPrint(lastQuotationHTML)}
//               >
...
//                 onClick={() => {
//                   const w = window.open("", "_blank");
//                   w.document.write(lastQuotationHTML);
//                   w.document.close();
//                   setTimeout(() => {
...
import { useState, useEffect, useCallback, useRef } from "react";
import {
  Plus, Trash2, Printer, Search, CheckCircle, AlertTriangle,
  Loader2, Download, Eye, X, EyeOff, ChevronDown,
} from "lucide-react";
...
function isTimberWood(item) {
  const text = `${item?.materialType || ""} ${item?.product || ""} ${item?.category || ""}`.toLowerCase();
  return (
    /timber|teak|sagwan|hardwood|oak|mahogany|pine|walnut|cedar|rosewood|maple|ash|birch|beech|reaper|batten|red\s*oak|white\s*oak/i.test(text) &&
    !/door/i.test(text) &&
    !isWoodenCladding(item) &&
    !isWoodenBaluster(item)
  );
...
function parseBalusterDimensions(name) {
  if (!name) return null;
  const sizeMatch = name.match(/(\d+(?:\.\d+)?)\s*["']?\s*[x×X]\s*(\d+(?:\.\d+)?)\s*["']?/i);
  if (!sizeMatch) return null;
  const width = parseFloat(sizeMatch[1]);
  const thickness = parseFloat(sizeMatch[2]);
  const lengthMatch = name.match(/[-–]\s*(\d+(?:\.\d+)?)\s*(feet|ft|foot)/i);
...
function parseSolidSurfaceDimensions(name) {
  ...
  if (mmMatch) thickness = parseFloat(mmMatch[1]);
  let cleanName = name.replace(/\d+(?:\.\d+)?\s*mm/gi, "").trim();
  const match = cleanName.match(/(\d+(?:\.\d+)?)\s*["']?\s*[x×X]\s*(\d+(?:\.\d+)?)/i);
  if (!match) return null;
  let w = parseFloat(match[1]);
  let h = parseFloat(match[2]);
  if (isNaN(w) || isNaN(h) || w === 0 || h === 0) return null;
  ...
  return {
    ...
    heightFeet: Math.round(hFeet * 10000) / 10000,
    areaPerPiece: Math.round(areaSqft * 1000) / 1000,
    thickness, widthOrig: w, heightOrig: h,
  };
}
...
function calculateByUnit(item) {
  ...
  if (isDoorFrame(item)) {
    if (item.unit === "Per Piece") return { calculatedQty: qty, amount: Math.round(qty * rate * 100) / 100 };
    const totalCFT = calculateDoorFrameCFT(item);
    return { calculatedQty: totalCFT, amount: Math.round(totalCFT * rate * 100) / 100 };
  }
  ...
  if (isCeilingPlank(item)) {
    ...
    let calculatedQty = qty;
    if (item.unit === "SQFT") calculatedQty = areaPerPc * qty;
    else if (item.unit === "RFT") calculatedQty = (customL || item.width || 0) * qty;
    else calculatedQty = qty;
  ...
  if (isWoodenBaluster(item)) {
    const width = parseFloat(item.width || 0);
    const thickness = parseFloat(item.thickness || 0);
    const lengthFeet = parseFloat(item.lengthFeet || 0) + parseFloat(item.lengthInches || 0) / 12;
    let calculatedQty = qty;
    if (item.unit === "CFT") calculatedQty = (width * thickness * lengthFeet * qty) / 144;
    else calculatedQty = qty;
    return { calculatedQty: Math.round(calculatedQty * 1000) / 1000, amount: Math.round(calculatedQty * rate * 100) / 100 };
  }
  ...
  if (item.isSheet) {
    ...
    if (item.unit === "Per Piece" || item.unit === "Pcs") {
      return { calculatedQty: qty, amount: Math.round(qty * rate * 100) / 100 };
    }
  }
  ...
  if (isTimberWood(item) || (item.isWood && !item.isSheet)) {
    ...
    let calculatedQty = qty;
    switch (item.unit) {
      case "CFT": calculatedQty = (width * thickness * lengthFeet * qty) / 144; break;
      case "RFT": calculatedQty = lengthFeet * qty; break;
      case "SQFT": calculatedQty = (width * lengthFeet * qty) / 12; break;
      case "Per Piece":
...
function SearchableSelect({ options, value, onChange, placeholder = "Search...", disabled = false, allowCustom = false, T, displayValue = "" }) {
  ...
  const filtered = options.filter((o) => getDisp(o).toLowerCase().includes((inputValue || "").toLowerCase()));
  const exactMatch = options.find((o) => getDisp(o).toLowerCase() === (inputValue || "").trim().toLowerCase());
  return (
    <div ref={wrapRef} className="searchable-select">
      <div className="ss-input-wrap">
        <input type="text" className="ss-input" placeholder={placeholder} value={inputValue}
      ...
      {isOpen && !disabled && (
        <div className="ss-dropdown" style={{ background: T?.cardBg, borderColor: T?.borderSoft }}>
          ...
          {allowCustom && inputValue.trim() && !exactMatch && (
            <div key="ss-custom-entry" className="ss-option"
              onClick={() => commit(inputValue.trim(), inputValue.trim())}
          ...
          {filtered.length === 0 && !(allowCustom && inputValue.trim()) && (
            <div key="ss-no-results" className="ss-no-results"
              style={{ padding: "10px", color: T?.textMuted, textAlign: "center" }}>
              No results
            </div>
          )}
        </div>
...
.page-wrapper:last-child{page-break-after:auto!important;}
.page-content{height:282mm!important;min-height:282mm!important;max-height:282mm!important;display:flex!important;flex-direction:column!important;overflow:hidden!important;}
.ktp-table-wrap{flex:1!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;}
table.items{height:100%!important;flex:1!important;}
table.items tbody{height:100%!important;}
table.items .spacer-row{height:100%!important;}
table.items .spacer-row td{height:100%!important;border-bottom:none!important;}
...
function getQuotationPrintHTML(order, quotation, hidePrice = false, chargesList = [], gstRate = 0, gstAmount = 0) {
  const regularItems = (quotation.items || []).filter((it) => !it.isCharge);
  const itemsTotal = regularItems.reduce((s, it) => s + (it.amount || 0), 0);
  const chargesTotal = chargesList.reduce((s, ch) => s + ch.amount, 0);
  const subTotalWithCharges = itemsTotal + chargesTotal;
  ...
  let chargesHtml = "";
  if (chargesList.length && !hidePrice) {
    chargesHtml = `<div class="ktp-charge-section"><div class="ktp-charge-header">ADDITIONAL CHARGES</div>${chargesList.map(ch => `<div class="ktp-charge-row"><span>${ch.name}</span><span class="ktp-charge-amt">₹${ch.amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span></div>`).join("")}</div>`;
  }
...
const apiPut = async (url, body) => {
  try { const r = await fetch(url, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); return await r.json(); }
  catch (e) { return { success: false, error: e.message }; }
};
const apiDelete = async (url, body) => {
  try { const r = await fetch(url, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); return r.json(); }
  catch (e) { return { success: false, error: e.message }; }
};
...
export default function QuotationOnlyBilling() {
  ...
  const [quotationForm, setQuotationForm] = useState({
    ...
    vehicleNo: "", quotationDate: new Date().toISOString().split("T")[0],
    gstRate: 0, notes: "", poNumber: "", gstCustomerName: "",
    hidePrice: false,
  });
  ...
  function createEmptyItem(ov = {}) {
    return {
      ...
      materialType: "", category: "", subCategory: "", specification: "",
      areaPerPiece: null, customLength: "", customWidth: "",
      dfSectionWidth: "", dfSectionThickness: "", dfHeightFt: "",
      dfHeightIn: "", dfWidthFt: "", dfWidthIn: "",
      dfVerticalQty: '', dfHorizontalQty: '', ...ov,
    };
  }
  ...
  const getAllItems = () =>
    itemGroups.flatMap((g) => g.items.map((i) => ({
      ...i, filterMaterialType: g.filterMaterialType, filterCategory: g.filterCategory, filterSubCategory: g.filterSubCategory,
    })));

  const itemsSubtotal = getAllItems().reduce((s, i) => s + (i.amount || 0), 0);
  const chargesSubtotal = charges.reduce((s, c) => s + (c.amount || 0), 0);
  const gstAmount = quotationForm.gstRate > 0 ? (itemsSubtotal + chargesSubtotal) * (quotationForm.gstRate / 100) : 0;
  const quotationTotal = itemsSubtotal + chargesSubtotal + gstAmount;
  ...
  const genQuotationNo = () => {
    const y = new Date().getFullYear();
    const prefix = `QT-${y}-`;
    const max = quotations.filter((q) => q.quotationNo?.startsWith(prefix))
      .reduce((m, q) => Math.max(m, parseInt(q.quotationNo.replace(prefix, "")) || 0), 0);
    return `${prefix}${String(max + 1).padStart(4, "0")}`;
  };
  ...
  const updateGroupFilter = (gid, field, val) =>
    setItemGroups((prev) => prev.map((g) => (g.groupId === gid ? { ...g, [field]: val } : g)));
  const addItemToGroup = (gid) =>
    setItemGroups((prev) => prev.map((g) => g.groupId === gid ? { ...g, items: [...g.items, createEmptyItem()] } : g));
  const removeItemFromGroup = (gid, iuid) =>
    setItemGroups((prev) => prev.map((g) => g.groupId === gid && g.items.length > 1 ? { ...g, items: g.items.filter((i) => i.uid !== iuid) } : g));
  const removeGroup = (gid) => { if (itemGroups.length > 1) setItemGroups((prev) => prev.filter((g) => g.groupId !== gid)); };
        ...
        return {
          ...
          items: g.items.map((item) => {
            ...
            if (field === "skuCode") {
              const found = products.find((p) => p.skuCode === val);
              if (found) {
                const isSheet = isSheetMaterial(found.materialName, found);
                const isWood = isTimberWood(found) && !isSheet;
                ...
                if (isSolidSurface(u)) {
                  u.unit = "SQFT";
                  const dims = parseSolidSurfaceDimensions(found.materialName);
                  if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = `${dims.widthOrig}"×${dims.heightOrig}"`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
                } else if (isCeilingPlank(u)) {
                  u.unit = "SQFT";
                  const dims = parseSheetDimensions(found.materialName);
                  if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = `${dims.widthFeet}'×${dims.heightFeet}'`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
                } else if (isWoodenBaluster(u)) {
                  u.unit = "Per Piece";
                  const dims = parseBalusterDimensions(found.materialName);
                  if (dims) { u.width = dims.width; u.thickness = dims.thickness; u.lengthFeet = dims.lengthFeet; u.lengthInches = ""; u.size = `${dims.width}"×${dims.thickness}" - ${dims.lengthFeet} ft`; }
                } else if (isWoodenCladding(u)) {
                  u.unit = "Per Piece";
                  const dims = parseWoodDimensions(found.materialName);
                  ...
                  u.unit = "CFT";
                  const dims = parseDoorFrameDimensions(found.materialName);
                  if (dims) { u.dfSectionWidth = dims.sectionWidth; u.dfSectionThickness = dims.sectionThickness; u.dfHeightFt = dims.heightFt; u.dfHeightIn = dims.heightIn; u.dfWidthFt = dims.widthFt; u.dfWidthIn = dims.widthIn; }
                } else if (isSheet) {
                  u.unit = "SQFT";
                  const dims = parseSheetDimensions(found.materialName);
                  if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = dims.isInches ? `${dims.widthOrig}"×${dims.heightOrig}"` : `${dims.widthFeet}'×${dims.heightFeet}'`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
                  ...
                  u.unit = found.unit || "Pcs"; u.width = 0; u.thickness = 0; u.size = ""; u.areaPerPiece = null;
                }
              } else if (!val) { u.product = ""; u.skuCode = ""; }
            }

            if (field === "customProductName" && val?.trim()) {
              const tempItem = { product: val, materialType: g.filterMaterialType || "", category: g.filterCategory || "", subCategory: g.filterSubCategory || "" };
              const isSheet = isSheetMaterial(val, tempItem);
              const isWood = isTimberWood(tempItem) && !isSheet;
              u.product = val; u.skuCode = ""; u.materialType = g.filterMaterialType || "Custom"; u.category = g.filterCategory || "Custom"; u.subCategory = g.filterSubCategory || ""; u.isWood = isWood; u.isSheet = isSheet;

              if (isSolidSurface(tempItem)) {
                u.unit = "SQFT"; const dims = parseSolidSurfaceDimensions(val);
                if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = `${dims.widthOrig}"×${dims.heightOrig}"`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
              } else if (isCeilingPlank(tempItem)) {
                u.unit = "SQFT"; const dims = parseSheetDimensions(val);
                if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = `${dims.widthFeet}'×${dims.heightFeet}'`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
              } else if (isWoodenBaluster(tempItem)) {
                u.unit = "Per Piece"; const dims = parseBalusterDimensions(val);
                if (dims) { u.width = dims.width; u.thickness = dims.thickness; u.lengthFeet = dims.lengthFeet; u.lengthInches = ""; u.size = `${dims.width}"×${dims.thickness}" - ${dims.lengthFeet} ft`; }
              } else if (isWoodenCladding(tempItem)) {
                u.unit = "Per Piece"; const dims = parseWoodDimensions(val);
                if (dims) { u.width = dims.width; u.thickness = dims.thickness; u.size = `${dims.width}×${dims.thickness}"`; }
              } else if (isDoorFrame(tempItem)) {
                u.unit = "CFT"; const dims = parseDoorFrameDimensions(val);
                if (dims) { u.dfSectionWidth = dims.sectionWidth; u.dfSectionThickness = dims.sectionThickness; u.dfHeightFt = dims.heightFt; u.dfHeightIn = dims.heightIn; u.dfWidthFt = dims.widthFt; u.dfWidthIn = dims.widthIn; }
              } else if (isSheet) {
                u.unit = "SQFT"; const dims = parseSheetDimensions(val);
                if (dims) { u.areaPerPiece = dims.areaPerPiece; u.width = dims.widthFeet; u.thickness = dims.thickness; u.size = dims.isInches ? `${dims.widthOrig}"×${dims.heightOrig}"` : `${dims.widthFeet}'×${dims.heightFeet}'`; u.customLength = dims.widthFeet; u.customWidth = dims.heightFeet; }
              } else if (isWood) {
                u.unit = /railing/i.test(val) ? "RFT" : "CFT"; const dims = parseWoodDimensions(val);
                if (dims) { u.width = dims.width; u.thickness = dims.thickness; u.size = `${dims.width}×${dims.thickness}"`; }
            ...
            if (field === "customLength") {
              u.customLength = val;
              const cl = parseFloat(val || 0); const cw = parseFloat(u.customWidth || 0);
              if (cl > 0 && cw > 0) { u.size = `${cl}'×${cw}'`; u.areaPerPiece = cl * cw; }
            }
            if (field === "customWidth") {
              u.customWidth = val;
              const cl = parseFloat(u.customLength || 0); const cw = parseFloat(val || 0);
              if (cl > 0 && cw > 0) { u.size = `${cl}'×${cw}'`; u.areaPerPiece = cl * cw; }
            }

            if (['dfSectionWidth', 'dfSectionThickness', 'dfHeightFt', 'dfHeightIn', 'dfWidthFt', 'dfWidthIn', 'dfVerticalQty', 'dfHorizontalQty'].includes(field)) u[field] = val;
  ...
  const updateCharge = (cuid, field, val) => {
    setCharges((prev) => prev.map((c) => {
      if (c.uid !== cuid) return c;
      let u = { ...c, [field]: val };
      if (field === "chargeType") { const ct = CHARGE_TYPES.find((t) => t.value === val); if (ct) u.chargeName = ct.label; }
      if (field === "unit" && val === "Lump Sum") u.quantity = "";
      u.amount = calculateChargeAmount(u);
      return u;
    }));
  ...
  const handleSubmitQuotation = async () => {
    if (!quotationForm.customerName || getAllItems().filter((i) => i.product).length === 0) {
      setError("Customer name aur at least ek item chahiye");
      return;
    }
    setSaving(true); setError(null);
    try {
      const quotationNo = editingQuotation ? editingQuotation.quotationNo : genQuotationNo();
      const validItems = getAllItems().filter((i) => i.product && (i.quantity || i.calculatedQty))
        .map((it) => ({
          product: it.product, unit: it.unit, quantity: it.quantity, rate: it.rate,
          amount: it.amount, calculatedQty: it.calculatedQty, size: it.size,
          lengthDisplay: (it.isWood || isTimberWood(it) || isWoodenCladding(it) || isWoodenBaluster(it)) ? `${it.lengthFeet || 0}'-${it.lengthInches || 0}"` : "",
          specification: it.specification || "", isSheet: it.isSheet, areaPerPiece: it.areaPerPiece,
          pieces: parseFloat(it.quantity || 0), sentQty: parseFloat(it.quantity || 0), orderedQty: it.calculatedQty,
        }));

      const validCharges = charges.filter((c) => c.chargeName && c.amount > 0)
        .map((ch) => ({ name: ch.chargeName, type: ch.chargeType || "custom", unit: ch.unit || "Per Piece", quantity: ch.quantity || "", rate: ch.rate || "", amount: ch.amount }));

      if (quotationForm.gstRate > 0 && gstAmount > 0) {
        validCharges.push({ name: `GST (${quotationForm.gstRate}%)`, type: "gst", unit: "Percentage", quantity: quotationForm.gstRate, rate: quotationForm.gstRate, amount: gstAmount });
      }

      const payload = {
        quotation: {
          quotationNo, customerName: quotationForm.customerName, customerPhone: quotationForm.customerPhone,
          customerAddress: quotationForm.customerAddress, vehicleNo: quotationForm.vehicleNo,
          poNumber: quotationForm.poNumber, gstCustomerName: quotationForm.gstCustomerName,
          quotationDate: quotationForm.quotationDate, deliveryNote: quotationForm.notes,
          quotationTotal, status: "Pending", hidePrice: quotationForm.hidePrice,
          gstRate: quotationForm.gstRate, gstAmount, subtotal: itemsSubtotal, chargesTotal: chargesSubtotal,
        },
        items: validItems, charges: validCharges,
      };

      let res;
      if (editingQuotation) res = await apiPut("/api/billing-backend/quotations", payload);
      else res = await apiPost("/api/billing-backend/quotations", payload);
      if (!res.success) throw new Error(res.error);

      const html = getQuotationPrintHTML(
        { customerName: quotationForm.customerName, customerPhone: quotationForm.customerPhone, customerAddress: quotationForm.customerAddress, vehicleNo: quotationForm.vehicleNo, poNumber: quotationForm.poNumber, gstCustomerName: quotationForm.gstCustomerName },
        { ...payload.quotation, items: validItems },
        quotationForm.hidePrice, validCharges.filter((c) => c.type !== "gst"), quotationForm.gstRate, gstAmount,
      );

      setLastQuotationHTML(html); setLastQuotationNo(quotationNo);
      await fetchData();
      setShowQuotationForm(false); setEditingQuotation(null); resetForm(); setShowSuccess(true);
  ...
  const handleDeleteQuotation = async (quotationNo) => {
    if (!confirm(`Delete quotation ${quotationNo}?`)) return;
    try {
      const res = await apiDelete("/api/billing-backend/quotations", { quotationNo });
      if (res.success) await fetchData();
      else setError(res.error);
    } catch (err) { setError(err.message); }
  };

  // ✅ FIXED: Edit me sab data properly fetch hoga
  const handleEditQuotation = (qt) => {
    setQuotationForm({
      customerName: qt.customerName || "", customerPhone: qt.customerPhone || "",
      customerAddress: qt.customerAddress || "", vehicleNo: qt.vehicleNo || "",
      quotationDate: qt.quotationDate || new Date().toISOString().split("T")[0],
      gstRate: parseFloat(qt.gstRate) || 0, notes: qt.deliveryNote || "",
      poNumber: qt.poNumber || "", gstCustomerName: qt.gstCustomerName || "",
      hidePrice: qt.hidePrice || false,
    });
    ...
    if (savedItems.length > 0) {
      const groupItems = savedItems.map((it) => {
        const tempItem = { product: it.product, materialType: it.materialType || it.product || "", category: it.category || it.product || "" };
        const isSheet = isSheetMaterial(it.product, tempItem);
        const isWood = isTimberWood(tempItem) && !isSheet;
        const isCladding = isWoodenCladding(tempItem);
        const isBaluster = isWoodenBaluster(tempItem);
        const isSolid = isSolidSurface(tempItem);
        const isCeiling = isCeilingPlank(tempItem);
        const isDoor = isDoorFrame(tempItem);

        const woodDims = (isWood || isCladding) && !isSheet ? parseWoodDimensions(it.product) : null;
        const balusterDims = isBaluster ? parseBalusterDimensions(it.product) : null;
        const sheetDims = isSheet ? parseSheetDimensions(it.product) : null;
        const solidDims = isSolid ? parseSolidSurfaceDimensions(it.product) : null;
        const doorDims = isDoor ? parseDoorFrameDimensions(it.product) : null;

        let lFeet = "";
        let lInches = "";
        if (it.lengthDisplay) {
          const lm = it.lengthDisplay.match(/(\d+(?:\.\d+)?)['\u2019]?\s*-?\s*(\d+(?:\.\d+)?)?["\u201D]?/);
          if (lm) { lFeet = lm[1] || ""; lInches = lm[2] || ""; }
        }

        let parsedWidth = 0, parsedThickness = 0, customL = "", customW = "";
        if (it.size) {
          const feetMatch = it.size.match(/(\d+(?:\.\d+)?)['\u2019]?\s*[×x]\s*(\d+(?:\.\d+)?)['\u2019]?/);
          const inchMatch = it.size.match(/(\d+(?:\.\d+)?)\s*[×x]\s*(\d+(?:\.\d+)?)["\u201D]?/);
          if (isSheet || isSolid || isCeiling) {
            if (feetMatch) { customL = feetMatch[1]; customW = feetMatch[2]; }
          } else if (isWood || isCladding || isBaluster) {
            if (inchMatch) { parsedWidth = parseFloat(inchMatch[1]) || 0; parsedThickness = parseFloat(inchMatch[2]) || 0; }
          }
        }

        if (!parsedWidth && woodDims) parsedWidth = woodDims.width;
        if (!parsedThickness && woodDims) parsedThickness = woodDims.thickness;
        if (!parsedWidth && balusterDims) parsedWidth = balusterDims.width;
        if (!parsedThickness && balusterDims) parsedThickness = balusterDims.thickness;
        if (!customL && sheetDims) customL = sheetDims.widthFeet;
        if (!customW && sheetDims) customW = sheetDims.heightFeet;
        if (!customL && solidDims) customL = solidDims.widthFeet;
        if (!customW && solidDims) customW = solidDims.heightFeet;
        if (!lFeet && balusterDims?.lengthFeet) lFeet = balusterDims.lengthFeet.toString();

        let areaPerPc = null;
        if (customL && customW) areaPerPc = parseFloat(customL) * parseFloat(customW);
        else if (sheetDims) areaPerPc = sheetDims.areaPerPiece;
        else if (solidDims) areaPerPc = solidDims.areaPerPiece;
        ...
        return createEmptyItem({
          ...
          amount: parseFloat(it.amount) || 0,
          calculatedQty: parseFloat(it.calculatedQty) || 0,
          size: it.size || "", lengthFeet: lFeet, lengthInches: lInches,
          isWood, isSheet, width: parsedWidth, thickness: parsedThickness,
          areaPerPiece: areaPerPc, customLength: customL, customWidth: customW,
          lengthDisplay: it.lengthDisplay || "", specification: it.specification || "",
          dfSectionWidth: doorDims?.sectionWidth || "",
          dfSectionThickness: doorDims?.sectionThickness || "",
          dfHeightFt: doorDims?.heightFt || "", dfHeightIn: doorDims?.heightIn || "",
          dfWidthFt: doorDims?.widthFt || "", dfWidthIn: doorDims?.widthIn || "",
          dfVerticalQty: 2, dfHorizontalQty: 1,
        });
      });

      setItemGroups([{ groupId: uid(), filterMaterialType: "", filterCategory: "", filterSubCategory: "", items: groupItems }]);
    } else {
      setItemGroups([createEmptyGroup()]);
    }
    ...
    const savedCharges = (qt.charges || []).filter((c) => c.type !== "gst");
    if (savedCharges.length > 0) {
      setCharges(savedCharges.map((c) => ({
        uid: uid(), chargeType: c.type || "", chargeName: c.name || "",
        chargeDescription: "", unit: c.unit || "Per Piece",
        quantity: c.quantity || "", rate: c.rate || "", amount: parseFloat(c.amount) || 0,
      })));
    } else { setCharges([]); }
  ...
  if (loading)
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: 40, minHeight: "100vh" }}>
  ...
  return (
    <div style={{ background: T.pageBg, minHeight: "100vh", padding: "20px", transition: "background-color 0.3s ease" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 16, marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 24, fontWeight: "bold", color: T.maroon, margin: 0 }}>Quotations</h2>
          <p style={{ color: T.textMuted, fontSize: 13, margin: "4px 0 0" }}>
            {quotations.length} quotation{quotations.length !== 1 ? "s" : ""} total
          </p>
        </div>
        <button className="btn-maroon" onClick={() => { setEditingQuotation(null); resetForm(); setShowQuotationForm(true); }}>
          <Plus size={16} /> New Quotation
        </button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ position: "relative" }}>
          ...
          <input className="kt-input" style={{ paddingLeft: 36 }} placeholder="Search quotation..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>

        {filteredQuotations.map((qt) => {
          const regularCharges = (qt.charges || []).filter((c) => c.type !== "gst");
          const gstEntry = (qt.charges || []).find((c) => c.type === "gst");
          const gstRate = parseFloat(qt.gstRate || gstEntry?.quantity || 0);
          const gstAmt = parseFloat(qt.gstAmount || gstEntry?.amount || 0);

          return (
            <div key={qt.quotationNo} className="kt-card" style={{ padding: 16 }}>
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                <div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ fontFamily: "monospace", fontWeight: "bold", color: T.maroon }}>{qt.quotationNo}</span>
                    <span className="status-pill" style={{ background: T.amberBg, color: T.amberColor, border: `1px solid ${T.amberBorder}` }}>
                      <span className="status-dot" style={{ background: "#f59e0b" }} />
                      {qt.status || "Pending"}
                    </span>
                    {qt.hidePrice && (<span className="status-pill" style={{ background: T.cream, color: T.maroon }}><EyeOff size={12} /> Hidden</span>)}
                    {gstRate > 0 && (<span className="status-pill" style={{ background: T.infoBg, color: T.infoColor, border: `1px solid ${T.infoBorder}` }}>GST {gstRate}%</span>)}
                  </div>
                  <p style={{ fontWeight: 600, margin: "4px 0", color: T.textDark }}>{qt.customerName}</p>
                  <p style={{ fontSize: 12, color: T.textMuted, margin: 0 }}>
                    {new Date(qt.quotationDate).toLocaleDateString()} • {(qt.items || []).length} items
                    {!qt.hidePrice && ` • ₹${parseFloat(qt.quotationTotal || 0).toLocaleString()}`}
                  </p>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button className="btn-white" style={{ padding: "5px 12px", fontSize: 12 }}
                    onClick={() => openPDFView(getQuotationPrintHTML(qt, qt, qt.hidePrice, regularCharges, gstRate, gstAmt))}>
                    <Eye size={12} /> View
                  </button>
                  <button className="btn-amber" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => handleEditQuotation(qt)}>
                    ✏️ Edit
                  </button>
                  <button className="btn-maroon" style={{ padding: "5px 12px", fontSize: 12 }}
                    onClick={() => openPDFPrint(getQuotationPrintHTML(qt, qt, qt.hidePrice, regularCharges, gstRate, gstAmt))}>
                    <Printer size={12} /> Print
                  </button>
                  <button className="icon-btn" style={{ color: T.errorColor }} onClick={() => handleDeleteQuotation(qt.quotationNo)}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        {filteredQuotations.length === 0 && (
          <div className="kt-card" style={{ padding: 56, textAlign: "center" }}>
            <p style={{ fontSize: 14, color: T.textMuted }}>No quotations found</p>
          </div>
        )}
      </div>
      ...
      {showQuotationForm && (
        <div style={{ position: "fixed", inset: 0, background: T.overlayBg, zIndex: 50, overflow: "auto", padding: 16, display: "flex", alignItems: "flex-start", justifyContent: "center" }}>
          <div style={{ background: T.modalBg, border: `1px solid ${T.borderSoft}`, borderRadius: 16, maxWidth: 1400, width: "100%", margin: "32px 0", boxShadow: `0 8px 32px ${T.shadowStrong}` }}>
            <div style={{ padding: 20, borderBottom: `1px solid ${T.borderSoft}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ fontWeight: "bold", fontSize: 20, margin: 0, color: T.textDark }}>
                {editingQuotation ? `Edit Quotation - ${editingQuotation.quotationNo}` : "New Quotation"}
              </h3>
              <button className="icon-btn" onClick={() => { setShowQuotationForm(false); setEditingQuotation(null); resetForm(); }}>
                <X size={24} />
              </button>
            </div>
            ...
            <div style={{ padding: 20, maxHeight: "70vh", overflowY: "auto" }}>
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
                  <input className="kt-input" placeholder="Customer Name *" value={quotationForm.customerName} onChange={(e) => setQuotationForm({ ...quotationForm, customerName: e.target.value })} />
                  <input className="kt-input" placeholder="Phone" value={quotationForm.customerPhone} onChange={(e) => setQuotationForm({ ...quotationForm, customerPhone: e.target.value })} />
                  <input className="kt-input" placeholder="Valid Till (e.g. 30 Days)" value={quotationForm.vehicleNo} onChange={(e) => setQuotationForm({ ...quotationForm, vehicleNo: e.target.value })} />
                  <input type="date" className="kt-input" value={quotationForm.quotationDate} onChange={(e) => setQuotationForm({ ...quotationForm, quotationDate: e.target.value })} />
                  <select className="kt-input" value={quotationForm.gstRate} onChange={(e) => setQuotationForm({ ...quotationForm, gstRate: parseFloat(e.target.value) })}>
                    {GST_OPTIONS.map((o) => (<option key={`gst-opt-${o.value}`} value={o.value}>{o.label}</option>))}
                  </select>
                  <input className="kt-input" placeholder="Ref Number" value={quotationForm.poNumber} onChange={(e) => setQuotationForm({ ...quotationForm, poNumber: e.target.value })} />
                  <input className="kt-input" placeholder="GST Customer Name" value={quotationForm.gstCustomerName} onChange={(e) => setQuotationForm({ ...quotationForm, gstCustomerName: e.target.value })} />
                  <textarea className="kt-input" rows="2" placeholder="Address" value={quotationForm.customerAddress} onChange={(e) => setQuotationForm({ ...quotationForm, customerAddress: e.target.value })} style={{ gridColumn: "span 2" }} />
                </div>
              </div>

              <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginBottom: 20, color: T.textDark }}>
                <input type="checkbox" checked={quotationForm.hidePrice} onChange={(e) => setQuotationForm({ ...quotationForm, hidePrice: e.target.checked })} />
                <EyeOff size={18} /> Hide Price on Quotation
              </label>
                  ...
                  return (
                    <div key={`group-${group.groupId}`} className="material-group">
                      <div className="material-group-header">
                        ...
                        {itemGroups.length > 1 && (<button className="icon-btn" onClick={() => removeGroup(group.groupId)}><Trash2 size={16} /></button>)}
                      </div>
                          ...
                          <div key={`item-${item.uid}`} className="item-subrow">
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                ...
                                {isTimberWood(item) && (<span className="unit-badge wood-badge">Timber Wood</span>)}
                                {isWoodenCladding(item) && (<span className="unit-badge wood-badge">Wooden Cladding</span>)}
                                {isWoodenBaluster(item) && (<span className="unit-badge wood-badge">Baluster</span>)}
                                {isSolidSurface(item) && (<span className="unit-badge wood-badge">Solid Surface</span>)}
                                {isCeilingPlank(item) && (<span className="unit-badge wood-badge">Ceiling Plank</span>)}
                                {item.isSheet && !isCeilingPlank(item) && !isSolidSurface(item) && (<span className="unit-badge wood-badge">Sheet/Door</span>)}
                                {isDoorFrame(item) && (<span className="unit-badge wood-badge">Door Frame</span>)}
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
                              <div style={{ gridColumn: "span 2", minWidth: 200 }}>
                                <SearchableSelect options={gp.map((p) => ({ value: p.skuCode, label: p.materialName }))} value={item.skuCode} displayValue={item.product || ""}
                                  onChange={(val) => {
                                  ...
                                  placeholder="Search or type product name" allowCustom T={T}
                                />
                              </div>

                              <div>
                                {isDoorFrame(item) ? (
                                  <select className="kt-input" value={item.unit || "CFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {DOOR_FRAME_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : isSolidSurface(item) ? (
                                  <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {SOLID_SURFACE_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : isCeilingPlank(item) ? (
                                  <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {CEILING_PLANK_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : isWoodenBaluster(item) ? (
                                  <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {WOODEN_BALUSTER_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : isWoodenCladding(item) ? (
                                  <select className="kt-input" value={item.unit || "Per Piece"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {WOODEN_CLADDING_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : isTimberWood(item) ? (
                                  <select className="kt-input" value={item.unit || "CFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {TIMBER_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : item.isSheet ? (
                                  <select className="kt-input" value={item.unit || "SQFT"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {SHEET_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                ) : (
                                  <select className="kt-input" value={item.unit || "Pcs"} onChange={(e) => updateGroupItem(group.groupId, item.uid, "unit", e.target.value)}>
                                    {[...new Set([item.unit, ...CUSTOM_UNIT_OPTIONS].filter(Boolean))].map((u) => (<option key={u} value={u}>{u}</option>))}
                                  </select>
                                )}
                              </div>
                              ...
                              {isDoorFrame(item) && (
                                <div style={{ gridColumn: "1 / -1", display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", padding: "8px 0" }}>
                                  ...
                                  <input type="number" className="kt-input" value={item.dfVerticalQty} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfVerticalQty", e.target.value)} style={{ width: 70 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted }}>H.Qty</span>
                                  <input type="number" className="kt-input" value={item.dfHorizontalQty} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfHorizontalQty", e.target.value)} style={{ width: 70 }} />
                                  <input type="number" step="0.5" className="kt-input" placeholder='Sec.Width (")' value={item.dfSectionWidth} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfSectionWidth", e.target.value)} style={{ width: 100 }} />
                                  <input type="number" step="0.5" className="kt-input" placeholder='Thick (")' value={item.dfSectionThickness} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfSectionThickness", e.target.value)} style={{ width: 100 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted, fontWeight: 600 }}>Height:</span>
                                  <input type="number" className="kt-input" placeholder="Ft" value={item.dfHeightFt} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfHeightFt", e.target.value)} style={{ width: 80 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted }}>ft</span>
                                  <input type="number" className="kt-input" placeholder="In" value={item.dfHeightIn} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfHeightIn", e.target.value)} style={{ width: 80 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted }}>in</span>
                                  <span style={{ fontSize: 13, color: T.textMuted, fontWeight: 600 }}>Width:</span>
                                  <input type="number" className="kt-input" placeholder="Ft" value={item.dfWidthFt} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfWidthFt", e.target.value)} style={{ width: 80 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted }}>ft</span>
                                  <input type="number" className="kt-input" placeholder="In" value={item.dfWidthIn} onChange={(e) => updateGroupItem(group.groupId, item.uid, "dfWidthIn", e.target.value)} style={{ width: 80 }} />
                                  <span style={{ fontSize: 13, color: T.textMuted }}>in</span>
                                </div>
                              )}

                              {(isTimberWood(item) || isWoodenCladding(item) || isWoodenBaluster(item) || (item.isWood && !item.isSheet)) && (
                                <div className="length-group">
                              ...
                              {(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.unit === "SQFT" && (
                                <div className="length-group" style={{ gridColumn: "span 2" }}>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="Length" value={item.customLength} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customLength", e.target.value)} style={{ width: 75 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>ft ×</span>
                                  <input type="number" step="0.01" className="kt-input length-input" placeholder="Width" value={item.customWidth} onChange={(e) => updateGroupItem(group.groupId, item.uid, "customWidth", e.target.value)} style={{ width: 75 }} />
                                  <span style={{ color: T.textMuted, fontSize: 12 }}>ft</span>
                                  {item.customLength && item.customWidth && (
                                    <span style={{ marginLeft: 8, padding: "2px 8px", background: T.cream, color: T.maroon, borderRadius: 6, fontSize: 11, fontWeight: 600 }}>
                                      = {(parseFloat(item.customLength) * parseFloat(item.customWidth)).toFixed(2)} sqft/pc
                                    </span>
                                  )}
                                </div>
                              )}
                              ...
                              <div>
                                <input type="number" className="kt-input" placeholder={(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.unit === "SQFT" ? "Pieces" : "Qty"} value={item.quantity} onChange={(e) => updateGroupItem(group.groupId, item.uid, "quantity", e.target.value)} />
                              </div>
                              <div>
                                <input type="number" className="kt-input" placeholder={(item.isSheet || isSolidSurface(item) || isCeilingPlank(item)) && item.unit === "SQFT" ? "Rate/SQFT" : "Rate"} value={item.rate} onChange={(e) => updateGroupItem(group.groupId, item.uid, "rate", e.target.value)} />
                              </div>
                              <div>
                            ...
                            {item.product && !quotationForm.hidePrice && (
                              <div className="calc-display">
                                <span style={{ fontWeight: 600 }}>
                                  {isDoorFrame(item) && `CFT: ${item.calculatedQty.toFixed(3)}`}
                                  {(isTimberWood(item) || isWoodenCladding(item) || isWoodenBaluster(item)) && `Total ${item.unit}: ${item.calculatedQty.toFixed(3)}`}
                                  {item.isSheet && item.unit === "SQFT" && (() => {
                                    const effectiveArea = parseFloat(item.customLength || 0) * parseFloat(item.customWidth || 0) || item.areaPerPiece || 0;
                                    return `${item.quantity || 0} pcs × ${effectiveArea.toFixed(2)} sqft = ${item.calculatedQty.toFixed(2)} SQFT`;
                                  })()}
                                  {item.isSheet && (item.unit === "Per Piece" || item.unit === "Pcs") && `Qty: ${item.calculatedQty} pcs`}
                                  {isSolidSurface(item) && `${item.quantity || 0} pcs = ${item.calculatedQty.toFixed(2)} SQFT`}
                                  {!isDoorFrame(item) && !isTimberWood(item) && !isWoodenCladding(item) && !isWoodenBaluster(item) && !item.isSheet && !isSolidSurface(item) && `Qty: ${item.quantity || 0}`}
                                </span>
                                {" • "}
                  ...
                  <div key={`charge-${ch.uid}`} style={{ padding: 12, border: `1px solid ${T.borderSoft}`, borderRadius: 10, marginBottom: 8, background: T.accent }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 6, fontSize: 12, background: T.amberBg, color: T.amberColor, fontWeight: 600 }}>
                        {CHARGE_TYPES.find((t) => t.value === ch.chargeType)?.icon || "📋"} #{idx + 1} {ch.chargeName || "New Charge"}
                      </span>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8 }}>
                      ...
                      <select className="kt-input" value={ch.unit} onChange={(e) => updateCharge(ch.uid, "unit", e.target.value)}>
                        {CHARGE_UNIT_OPTIONS.map((u) => (<option key={u} value={u}>{u}</option>))}
                      </select>
                      {ch.unit !== "Lump Sum" && (
                ...
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <div className="total-box" style={{ width: 320 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, color: T.textDark }}>
                      <span>Materials</span><span>₹{itemsSubtotal.toLocaleString()}</span>
                    </div>
                    {chargesSubtotal > 0 && (
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, color: T.textDark }}>
                    ...
                    {quotationForm.gstRate > 0 && (
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, color: T.textDark }}>
                        <span>GST ({quotationForm.gstRate}%)</span><span>₹{gstAmount.toLocaleString()}</span>
                      </div>
                    )}
                    <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: 18, borderTop: `2px solid ${T.maroon}`, paddingTop: 8, marginTop: 8, color: T.maroon }}>
                      <span>Grand Total</span><span>₹{quotationTotal.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div style={{ padding: 20, borderTop: `1px solid ${T.borderSoft}`, display: "flex", justifyContent: "flex-end", gap: 12 }}>
              <button className="btn-white" onClick={() => { setShowQuotationForm(false); setEditingQuotation(null); resetForm(); }}>Cancel</button>
              <button className="btn-maroon" disabled={saving} onClick={handleSubmitQuotation}>
                {saving ? (<Loader2 size={16} className="animate-spin" />) : ("Save Quotation")}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccess && (
        <div style={{ position: "fixed", inset: 0, background: T.overlayBg, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: T.modalBg, borderRadius: 20, padding: 32, textAlign: "center", maxWidth: 420, width: "90%", boxShadow: `0 8px 40px ${T.shadowStrong}` }}>
            ...
            <h3 style={{ fontSize: 22, fontWeight: "bold", marginBottom: 4, color: T.textDark }}>
              {editingQuotation ? "Quotation Updated!" : "Quotation Created!"}
            </h3>
            <p style={{ fontFamily: "monospace", margin: "8px 0 20px", color: T.maroon, fontSize: 16, fontWeight: 600 }}>{lastQuotationNo}</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 16 }}>
              <button className="btn-teal" onClick={() => openPDFView(lastQuotationHTML)}><Eye size={16} /> View</button>
              <button className="btn-maroon" onClick={() => openPDFPrint(lastQuotationHTML)}><Printer size={16} /> Print</button>
              <button className="btn-blue" onClick={() => {
                const w = window.open("", "_blank");
                w.document.write(lastQuotationHTML); w.document.close();
                setTimeout(() => {
                  const ab = w.document.querySelector(".action-bar");
                  if (ab) ab.style.display = "none";
                  w.print();
            ...
            <button className="btn-white" style={{ width: "100%", justifyContent: "center" }} onClick={() => setShowSuccess(false)}>Close</button>
          </div>
        </div>
      )}
    </div>
